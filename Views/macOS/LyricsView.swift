...
// MARK: - LyricsView with AI (macOS 26+)

#if canImport(FoundationModels)
    /// Extended lyrics view with AI explanation features for macOS 26+.
    @available(macOS 26.0, *)
    struct LyricsViewWithAI: View {
        @Environment(PlayerService.self) private var playerService
...
// MARK: - Adaptive Lyrics Container

/// Container that uses AI-enabled lyrics view on macOS 26+ and basic view on older systems.
@available(macOS 15.0, *)
struct AdaptiveLyricsView: View {
    let client: any YTMusicClientProtocol

    var body: some View {
        #if canImport(FoundationModels)
            if #available(macOS 26.0, *) {
                LyricsViewWithAI(client: self.client)
            } else {
